...
const ShowExpense = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        ///////////////
                       ...
                       const expenseWithUserId = Object.values(data).find(expense => expense.userId === userId);

        if (!expenseWithUserId) {
            throw new Error(`No expense found for userId: ${userId}`);
        }

        // Extract the id from the found expense object
        const id = Object.keys(data).find(key => data[key].userId === userId);
  ...
  const deleteHandler = async (id) => {
    try {
      ...
      // console.log('>>>>  expenseId DELETE ', id); // Log the id of the expense to be deleted

      const response = await fetch(
        `https://ajay-singh-f6aa8-default-rtdb.firebaseio.com/Expense/${userId}/${`-NsitsD0w5snINhB6H3D
        `}.json`,
        {
          method: "DELETE",
        }
      ...
      // Dispatch deleteExpense action with the id
      dispatch(expenseActions.deleteExpense(id));

      // Add the deleted expense back to the expenses list
  ...
  const clickActPremiumHandler = async () => {
    try {
      ...
      console.log(">>>> Firebase data", dataFirebase);
      if (!resFirebase.ok) {
